  ...
  */
  ...
  CCC.CURRENT.unpack = function (value) {
    ...
    var valuesArrayLenght = valuesArray.length;
    var mask = valuesArray[valuesArrayLenght - 1];
    var maskInt = parseInt(mask, 16);//hexadecimal
    var unpackedCurrent = {};
    var currentField = 0;
    for (var property in this.FIELDS) {
      if (this.FIELDS[property] === 0) {
      ...
      else if (maskInt & this.FIELDS[property]) {//The Bitwise AND (&) operator will return 1 for a bit if both operand bits are equal to "1", otherwise it will return 0 (zero).
