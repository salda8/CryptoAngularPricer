...
function queryParams(params: string[]) {
  return Object.keys(params)
    .map(k => `${encodeURIComponent(k)}=${encodeURIComponent(params[k])}`)
    .join("&");
}

function withQuery(url: string, params: string[]) {
...
function errorResponse(xhr: XMLHttpRequest, message?: string): RequestResult {
  return {
    ok: false,
    status: xhr.status,
    statusText: xhr.statusText,
...
export function request(
  ...
  return new Promise<RequestResult>((resolve, reject) => {
    ...
    xhr.open(method, withQuery(url, queryParameters));
