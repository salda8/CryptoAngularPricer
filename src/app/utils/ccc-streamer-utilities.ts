...
export class StreamerUtils {
  ...
  */
  ...
  getTradeFieldEnumFromString(property: string): TRADE_FIELDS {
    switch (property) {
      ...
      case "M":
        return TRADE_FIELDS.M;
      case "FSYM":
        return TRADE_FIELDS.FSYM;
      case "TSYM":
        return TRADE_FIELDS.TSYM;
      case "F":
        return TRADE_FIELDS.F;
      case "ID":
        return TRADE_FIELDS.ID;
      case "TS":
        return TRADE_FIELDS.TS;
      case "Q":
        return TRADE_FIELDS.Q;
      case "P":
        return TRADE_FIELDS.P;
      case "TOTAL":
        return TRADE_FIELDS.TOTAL;
      default:
  ...
  getFlagEnumFromString(property: string): FLAGS {
    switch (property) {
      ...
      case "PRICEDOWN":
        return FLAGS.PRICEDOWN;
      case "PRICEUNCHANGED":
        return FLAGS.PRICEUNCHANGED;
      case "BIDUP":
        return FLAGS.BIDUP;
      case "BIDDOWN":
        return FLAGS.BIDDOWN;
      case "BIDUNCHANGED":
        return FLAGS.BIDUNCHANGED;
      case "OFFERUP":
        return FLAGS.OFFERUP;
      case "OFFERDOWN":
        return FLAGS.OFFERDOWN;
      case "OFFERUNCHANGED":
        return FLAGS.OFFERUNCHANGED;
      case "AVGUP":
        return FLAGS.AVGUP;
      case "AVGDOWN":
        return FLAGS.AVGDOWN;
      case "AVGUNCHANGED":
  ...
  getFieldEnumFromString(property: string): FIELDS {
    switch (property) {
      ...
      case "FROMSYMBOL":
        return FIELDS.FROMSYMBOL;
      case "TOSYMBOL":
        return FIELDS.TOSYMBOL;
      case "FLAGS":
        return FIELDS.FLAGS;
      case "PRICE":
        return FIELDS.PRICE;
      ...
      case "OFFER":
        return FIELDS.OFFER;
      case "LASTUPDATE":
        return FIELDS.LASTUPDATE;
      case "AVG":
        return FIELDS.AVG;
      case "LASTVOLUME":
        return FIELDS.LASTVOLUME;
      case "LASTVOLUMETO":
        return FIELDS.LASTVOLUMETO;
      case "LASTTRADEID":
        return FIELDS.LASTTRADEID;
      case "VOLUMEHOUR":
        return FIELDS.VOLUMEHOUR;
      case "VOLUMEHOURTO":
        return FIELDS.VOLUMEHOURTO;
      case "VOLUME24HOUR":
        return FIELDS.VOLUME24HOUR;
      case "VOLUME24HOURTO":
        return FIELDS.VOLUME24HOURTO;
      case "OPENHOUR":
        return FIELDS.OPENHOUR;
      case "HIGHHOUR":
        return FIELDS.HIGHHOUR;
      case "LOWHOUR":
        return FIELDS.LOWHOUR;
      case "OPEN24HOUR":
        return FIELDS.OPEN24HOUR;
      case "HIGH24HOUR":
        return FIELDS.HIGH24HOUR;
      case "LOW24HOUR":
        return FIELDS.LOW24HOUR;
      case "LASTMARKET":
        return FIELDS.LASTMARKET;
      default:
  ...
  unpack(tradeString) {
    ...
    for (const property in TRADE_FIELDS) {
      // console.log("PROPERTY: ", property);
      const tradeField = this.getTradeFieldEnumFromString(property);
      if (tradeField === 0) {
  ...
  */
  ...
  current_pack(currentObject) {
    ...
    for (const property in FIELDS) {
      if (currentObject.hasOwnProperty(property)) {
        ...
        mask |= this.getFieldEnumFromString(property);
      }
    }
  ...
  current_unpack(value) {
    ...
    // tslint:disable-next-line:forin
    for (const property in FIELDS) {
      const field = this.getFieldEnumFromString(property);
      if (field === 0) {
  ...
  current_convertValueToDisplay(symbol, value, type?, fullNumbers?) {
    ...
    if (type === "short") {
      if (valueAbs > 10000000000) {
        valueAbs = valueAbs / 1000000000;
        return prefix + this.current_filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnBigNumbers) + " B";
      }
      if (valueAbs > 10000000) {
        valueAbs = valueAbs / 1000000;
        return prefix + this.current_filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnBigNumbers) + " M";
      }
      if (valueAbs > 10000) {
        valueAbs = valueAbs / 1000;
        return prefix + this.current_filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnBigNumbers) + " K";
      }
      if (valueAbs >= 1) {
        return prefix + this.current_filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnNormalNumbers);
      }
      return prefix + (valueSign * valueAbs).toPrecision(decimalsOnSmallNumbers);
    } else {
      if (valueAbs >= 1) {
      ...
      return prefix + this.current_noExponents((valueSign * valueAbs).toPrecision(decimalsOnSmallNumbers));
    }
  }
